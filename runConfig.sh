...
#
# geoschem_config.yml
Turn_on_Chemistry=true
Turn_on_Transport=true
Turn_on_Cloud_Conv=true
Turn_on_PBL_Mixing=true
Turn_on_Non_Local_Mixing=true
Turn_on_Dry_Deposition=true
Turn_on_Wet_Deposition=true
#
# HEMCO_Config.rc:
...
# NOTE: fullchem "DefaultCollection" excludes monthly option
# in HISTORY.rc by default to allow greater output frequency
# when other collections are monthly.
timeAvg_monthly="1"
...
#   (1) GEOS-Chem core: Set debug_printout to true in geoschem_config.yml
...
#    Timesteps
#------------------------------------------------
# WARNING: these settings will override settings in geoschem_config.yml!
...
if [[ ${Turn_on_Transport} == 'true' ]]; then
    ADVCORE_ADVECTION=1
elif [[ ${Turn_on_Transport} == 'false' ]]; then
    ADVCORE_ADVECTION=0
else
...
#### Set simulation start and end datetimes based on geoschem_config.yml
print_msg " "
...
print_msg "------------------"
replace_val_yaml "chemistry"         "activate"            "${Turn_on_Chemistry}"        geoschem_config.yml
replace_val_yaml "transport"         "activate"            "${Turn_on_Transport}"        geoschem_config.yml
replace_val_yaml "convection"        "activate"            "${Turn_on_Cloud_Conv}"       geoschem_config.yml
replace_val_yaml "pbl_mixing"        "activate"            "${Turn_on_PBL_Mixing}"       geoschem_config.yml
replace_val_yaml "pbl_mixing"        "use_non_local_pbl"   "${Turn_on_Non_Local_Mixing}" geoschem_config.yml
replace_val_yaml "dry_deposition"    "activate"            "${Turn_on_Dry_Deposition}"   geoschem_config.yml
replace_val_yaml "wet_deposition"    "activate"            "${Turn_on_Wet_Deposition}"   geoschem_config.yml
